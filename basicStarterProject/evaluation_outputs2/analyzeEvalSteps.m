...
evalData = readtable("eval_steps.csv");
evalData.Properties.VariableNames = {'episode', 'step', 'action', 'reward', 'total_reward', 'terminated', 'truncated', ...
...
for eIdx = 1:length(episodes)
    ...
    % Rward Plot
    subplot(6, 1, 1)
    plot(thisEpisodeData.step, thisEpisodeData.reward, 'LineWidth', 2, 'DisplayName','reward')
    hold on
    plot(thisEpisodeData.step, thisEpisodeData.total_reward, 'LineWidth', 2, 'DisplayName','totalReward')
    grid on
    grid minor
    xlabel("Step")
    ylabel("Reward")
    ...
    if (false)
        ...
        subplot(6, 1, 2)
        scatter(thisEpisodeData.step, actionData)
        xlabel("Step")
        ylabel("Action")
        yticks([0 1])
        ...
        grid minor
         ...
         % Action Plot
        subplot(6, 1, 2)
        plot(thisEpisodeData.step, thisEpisodeData.action, 'LineWidth', 2)
        xlabel("Step")
        ylabel("Action")
        yticks([0:1:5])
    ...
    subplot(6, 1, 3)
    scatter(thisEpisodeData.step, chosen_rank, 'LineWidth', 2)
    xlabel("Step")
    ylabel("Action Rank")
    grid on
    grid minor
    ...
    % Resource Fractions
    subplot(6, 1, 4)
    plot(thisEpisodeData.step, thisEpisodeData.battery_charge_fraction, 'LineWidth', 2, 'DisplayName','BatteryFraction')
    hold on
    plot(thisEpisodeData.step, thisEpisodeData.storage_level_fraction, 'LineWidth', 2, 'DisplayName','StorageFraction')
    hold on
    grid on
    grid minor
    xlabel("Step")
    ...
    scatter(inEclipse.step, ones(length(inEclipse.step), 1))
    xlabel("Step")
    ...
    plot(thisEpisodeData.step, isOpen)
